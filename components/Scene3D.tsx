...
import { Center, Grid, Points, PointMaterial, Stars, Text3D } from "@react-three/drei";
import { Suspense, useMemo, useRef, useState } from "react";
import * as THREE from "three";
import helvetikerBold from "three/examples/fonts/helvetiker_bold.typeface.json";

type SceneVariant = "hero" | "about" | "experience" | "work" | "services" | "testimonials" | "contact" | "footer";
...
function ParticleField({ count, radius, color }: { count: number; radius: number; color: string }) {
  ...
  return (
    <Points ref={ref} positions={positions} stride={3} frustumCulled>
      <PointMaterial color={color} transparent opacity={0.78} size={0.043} sizeAttenuation depthWrite={false} />
    </Points>
  );
}

function NetworkLines({ color, radius = 3.6, count = 34 }: { color: string; radius?: number; count?: number }) {
  const positions = useMemo(() => {
    ...
    nodes.forEach((node, index) => {
      ...
      if (index % 2 === 0) lines.push(node.x, node.y, node.z, near.x, near.y, near.z);
    });
...
function HologramPanel({ color, position, width = 4.2, height = 1.55 }: { color: string; position: [number, number, number]; width?: number; height?: number }) {
...
function SectionText({ variant, position = [0, 0, 0] }: { variant: SceneVariant; position?: [number, number, number] }) {
  ...
  useFrame((state) => {
    if (!ref.current) return;
    ref.current.rotation.y = THREE.MathUtils.lerp(ref.current.rotation.y, hovered ? state.pointer.x * 0.7 : Math.sin(state.clock.elapsedTime * 0.6) * 0.12, 0.06);
    ref.current.rotation.x = THREE.MathUtils.lerp(ref.current.rotation.x, hovered ? -state.pointer.y * 0.35 : 0, 0.06);
  ...
  return (
    <group ref={ref} position={position} onPointerOver={() => setHovered(true)} onPointerOut={() => setHovered(false)}>
      <mesh position={[0, 0, -0.16]} receiveShadow>
        ...
        <meshBasicMaterial color="#020617" transparent opacity={hovered ? 0.72 : 0.58} side={THREE.DoubleSide} />
      </mesh>
      <Center>
        <Text3D
          font={extrudedFont}
        ...
        >
          {labels[variant]}
          <meshStandardMaterial color="#e0f2fe" roughness={0.16} metalness={0.62} emissive={palette[0]} emissiveIntensity={hovered ? 0.55 : 0.28} />
        </Text3D>
      </Center>
      <Center position={[0.035, -0.035, -0.07]}>
        <Text3D font={extrudedFont} size={size} height={0.035} curveSegments={8} bevelEnabled bevelThickness={0.006} bevelSize={0.004} bevelSegments={2}>
          {labels[variant]}
...
function Shape({
  ...
  useFrame((state) => {
    ...
    ref.current.position.y = position[1] + Math.sin(state.clock.elapsedTime * speed + position[0]) * 0.16;
  ...
  return (
    <FloatShell speed={1.25 * speed}>
      <mesh ref={ref} position={position} scale={scale} onPointerOver={() => setHovered(true)} onPointerOut={() => setHovered(false)}>
        {geometry === "box" ? <boxGeometry args={[0.7, 0.7, 0.7]} /> : null}
        {geometry === "ico" ? <icosahedronGeometry args={[0.6, 1]} /> : null}
        {geometry === "dodeca" ? <dodecahedronGeometry args={[0.62, 0]} /> : null}
        {geometry === "octa" ? <octahedronGeometry args={[0.62, 0]} /> : null}
        {geometry === "torus" ? <torusKnotGeometry args={[0.45, 0.13, 110, 14]} /> : null}
        <meshStandardMaterial color={color} roughness={0.18} metalness={0.55} emissive={color} emissiveIntensity={hovered ? 0.2 : 0.05} />
      </mesh>
    </FloatShell>
  );
}

function FloatShell({ children, speed }: { children: React.ReactNode; speed: number }) {
  const ref = useRef<THREE.Group>(null);
  useFrame((state) => {
    if (!ref.current) return;
    ref.current.position.y = Math.sin(state.clock.elapsedTime * speed) * 0.08;
    ref.current.rotation.z = Math.sin(state.clock.elapsedTime * speed * 0.7) * 0.035;
  });
...
function ParallaxGroup({ children }: { children: React.ReactNode }) {
  ...
  useFrame((state) => {
    if (!ref.current) return;
    ref.current.rotation.y = THREE.MathUtils.lerp(ref.current.rotation.y, state.pointer.x * 0.25, 0.035);
    ref.current.rotation.x = THREE.MathUtils.lerp(ref.current.rotation.x, -state.pointer.y * 0.16, 0.035);
    ref.current.position.x = THREE.MathUtils.lerp(ref.current.position.x, state.pointer.x * 0.24, 0.03);
  });
  return <group ref={ref}>{children}</group>;
}
...
function HeroWorld() {
  ...
  return (
    <ParallaxGroup>
      ...
      <HologramPanel color={c[1]} position={[0.1, 0.15, 0.78]} width={3.6} height={1.35} />
      <SectionText variant="hero" position={[0.2, 0.15, 1.05]} />
      <Shape geometry="torus" position={[-2.7, 1.1, -1.4]} color={c[0]} scale={1.05} speed={1.1} />
      <Shape geometry="ico" position={[2.3, -0.3, -1]} color={c[1]} scale={0.9} speed={1.35} />
      <Shape geometry="dodeca" position={[0.8, 1.9, -2.1]} color={c[2]} scale={0.72} speed={0.9} />
      <Shape geometry="octa" position={[-0.8, -1.35, -1.2]} color={c[0]} scale={0.64} speed={1.55} />
    </ParallaxGroup>
  );
}
      ...
      {[0, 1, 2, 3, 4, 5].map((item) => {
        ...
        return (
          <Shape
            ...
            position={[Math.cos(angle) * 2.1, Math.sin(angle) * 0.85, -1.2 + Math.sin(angle) * 0.4]}
            color={c[item % 3]}
            scale={0.42}
            speed={0.8 + item * 0.1}
          />
  ...
  return (
    <ParallaxGroup>
      ...
      {[-1.8, -0.6, 0.6, 1.8].map((x, index) => (
        <Shape key={x} geometry="box" position={[x, -0.3 + (index % 2) * 0.45, -1.3]} color={c[index % 3]} scale={0.56} speed={1 + index * 0.18} />
      ))}
      <Grid position={[0, -1.35, -1.8]} args={[5.5, 3.2]} cellSize={0.45} cellThickness={0.45} cellColor={c[1]} sectionColor={c[0]} fadeDistance={5} fadeStrength={1.2} />
    </ParallaxGroup>
  );
}
...
function WorkWorld() {
  ...
  return (
    <ParallaxGroup>
      ...
      <Grid position={[0, -0.95, -1.9]} args={[7, 4]} cellSize={0.38} cellThickness={0.35} cellColor={c[1]} sectionColor={c[0]} fadeDistance={6} fadeStrength={1} />
      {[-1.6, 0, 1.6].map((x, index) => (
        <mesh key={x} position={[x, 0.05, -1.2]} rotation={[0.12, -0.2 + index * 0.2, 0]}>
          <boxGeometry args={[1.05, 0.62, 0.06]} />
          <meshStandardMaterial color={c[index]} roughness={0.24} metalness={0.45} emissive={c[index]} emissiveIntensity={0.08} />
        </mesh>
      ))}
    </ParallaxGroup>
  );
    ...
    <ParallaxGroup>
      ...
      <group ref={ring}>
        {[1.2, 1.8, 2.45].map((radius, index) => (
          <mesh key={radius} rotation={[Math.PI / 2 + index * 0.35, index * 0.62, 0]} position={[0, -0.05, -1.2]}>
            <torusGeometry args={[radius, 0.012, 16, 180]} />
            <meshBasicMaterial color={c[index]} transparent opacity={0.34 - index * 0.05} />
          </mesh>
        ))}
      </group>
...
function DefaultWorld({ variant }: { variant: SceneVariant }) {
  ...
  return (
    <ParallaxGroup>
      ...
      <Shape geometry="octa" position={[-1.7, -0.2, -1.4]} color={c[0]} scale={0.45} speed={1.1} />
      <Shape geometry="torus" position={[1.6, 0.2, -1.1]} color={c[1]} scale={0.48} speed={0.9} />
    </ParallaxGroup>
  );
}
...
function World({ variant }: { variant: SceneVariant }) {
  ...
  return (
    <>
      ...
      <spotLight position={[0, 3.5, 4]} angle={0.42} penumbra={0.65} intensity={2.4} color="#ffffff" castShadow />
      ...
      {["experience", "testimonials", "footer"].includes(variant) ? <DefaultWorld variant={variant} /> : null}
    </>
  );
}

function SceneCanvas({ variant, opacity = "opacity-60" }: { variant: SceneVariant; opacity?: string }) {
  return (
    <div className={`pointer-events-auto absolute inset-0 ${opacity}`}>
      <Canvas shadows camera={{ position: [0, 0.55, variant === "hero" ? 6.2 : 5.5], fov: variant === "hero" ? 48 : 54 }} dpr={[1, 1.6]}>
...
export function SectionScene({ variant }: { variant: SceneVariant }) {
  return <SceneCanvas variant={variant} opacity={variant === "footer" ? "opacity-60" : "opacity-70"} />;
}
